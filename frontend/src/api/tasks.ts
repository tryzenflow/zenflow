...
/**
 * No `status` param — the backend doesn't filter by status; a caller that
 * needs status filtering does it client-side over the returned `sessions`
 * array.
 */
 ...
 * resize. A change to a scheduled TASK's start/duration is recorded
 * server-side as a `MOVE` signal; there is no completion state. `scope` /
 * `skipConflicting` narrow a change to a series member.
 */
...
/**
 * Delete a session. For a recurring occurrence pass its
 * `"<seriesId>::<startISO>"` id — the backend excludes just that date and the
...
/** Delete a whole series (every occurrence/sitting + the series row). */
...
/**
 * "Delete this occurrence and every one after it" for a recurring (rrule)
 * series — the backend pulls the series' RRULE `UNTIL` back to just before
 * `fromStartISO`.
 */
 ...
 * `sessionIndex` order, keeping earlier sittings.
 */
