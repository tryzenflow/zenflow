...
describe("PythonPlacer.shadowCompareSingle", () => {
  const base = { user, task, now, legacyLinucbStart: null };

  it("stays quiet when the heuristic picks agree", async () => {
    const { placer } = make({ place: ok([member()]) });
    await expect(
      placer.shadowCompareSingle({
        ...base,
        legacyHeuristicStart: new Date(START),
      }),
    ).resolves.toBeUndefined();
  });

  it("never throws, even when the gateway blows up or Python is down", async () => {
    const a = make({ place: down("timeout") });
    await expect(
      a.placer.shadowCompareSingle({ ...base, legacyHeuristicStart: null }),
    ).resolves.toBeUndefined();
    const b = make({ place: ok([member()]) });
    b.gateway.buildRequest.mockRejectedValue(new Error("db down"));
    await expect(
      b.placer.shadowCompareSingle({ ...base, legacyHeuristicStart: null }),
    ).resolves.toBeUndefined();
  });

  it("writes nothing (no session update, no proposal)", async () => {
    const { placer, prisma, experiment } = make({
      place: ok([
        member({ heuristic: { startMs: START + 900_000, score: 1 } }),
      ]),
    });
    await placer.shadowCompareSingle({
      ...base,
      legacyHeuristicStart: new Date(START),
    });
    expect(prisma.session.update).not.toHaveBeenCalled();
