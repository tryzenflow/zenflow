...
def optimize_function(
    ...
    energy_weight=100,
    switch_penalty_weight=40,
    overload_weight=60,
    deviation_weight=40,
    min_gap_penalty_weight=200,
    available_hours_penalty_weight=200,
):
    loss_terms = []
    effective_durations = []
    ...
    for task_var in task_vars:
        ...
        if task.deadline is not None:
            penalty = model.NewIntVar(
                0, max_time * deadline_weight, f"deadline_penalty_{task.id}_{split}"
            )
        ...
        for i, block in enumerate(constraints.focus_blocks):
            ...
            if mismatch > 0:
                ...
                bonus = model.NewIntVar(
                    -energy_weight * task.duration, 0, f"energy_bonus_{task.id}_{i}"
                )
        ...
        if not task.mandatory:
            bonus = model.NewIntVar(
                -optional_task_weight, 0, f"bonus_presence_{task.id}"
            )
    ...
    model.Add(
        overload
        >= total_eff - model.NewConstant(constraints.max_daily_load - daily_load)
    )
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            gap_pos = model.NewIntVar(
                -max_time, max_time, f"gap_{i}_{j}_pos"
            )  # start_j - end_i
            model.Add(gap_pos == start_j - end_i)
            gap_neg = model.NewIntVar(
                -max_time, max_time, f"gap_{i}_{j}_neg"
            )  # start_i - end_j
            model.Add(gap_neg == start_i - end_j)

            # violation = max(0, min_gap - gap) when that orientation & both_present
            min_gap_val = (
                constraints.min_gap_between_tasks
                if hasattr(constraints, "min_gap_between_tasks")
                else 0
            )
            if min_gap_val > 0:
